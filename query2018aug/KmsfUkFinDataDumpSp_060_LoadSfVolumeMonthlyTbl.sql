...
/****** Object:  StoredProcedure [kmsf].[KmsfUkFinDataDumpSp_060_LoadSfVolumeMonthlyTbl]    Script Date: 8/8/2018 8:19:55 AM ******/
SET ANSI_NULLS ON
GO
...
SELECT EncMRN = cdv.EncMRN  --1
  ...
  ,BadDebtFlag = cast(null as varchar(10)) --67
    ,PrimPyrHlthPlanRptGrpMne = cdv.PrimPyrHlthPlanRptGrpMne --62
  ,PrimPyrPmtAmt = cast(null as decimal(18,2))--64
  ,PrimPyrAdjAmt = cast(null as decimal(18,2))--65
  ,PrimPyrBalAmt = cast(null as decimal(18,2))--66
  ,RcvOwnerShortName = cdv.ROShortName--63
...
Create nonclustered Index idx_chgobjid on  Staging.UKFinSfVolumeMonthly (ChgObjID)
Create nonclustered Index idx_glengobjid on  Staging.UKFinSfVolumeMonthly (GlEntObjID)

If OBJECT_ID('tempdb..#RcvData','U') is not null
Drop table #RcvData
Select m.GlEntObjId
			  ,PrimPyrPmtAmt =sum(isnull((case when r.PrimPyrRcvInd = 1 then r.TotPayAmt else 0.00 end),0.00))
			  ,PrimPyrAdjAmt = sum(isnull((case when r.PrimPyrRcvInd = 1 then r.TotAdjAmt else 0.00 end),0.00))
			  ,PrimPyrBalAmt = sum(isnull((case when r.PrimPyrRcvInd = 1 then r.BalAmt else 0.00 end),0.00))
			  ,BDIind = sum(isnull(case when r.RcvBlStsMne = 'BD' then 1 else 0 end,0))
Into #RcvData
From Staging.UKFinSfVolumeMonthly m
	left join KMSFDSSDBP01.smsphdssf240.smsdss.RcvFctV r on r.ChgObjId = m.ChgObjId and r.PrcsStsCd = 1
Group by m.GlEntObjId

Create nonclustered index idx_RcvGlEntObjID on #RcvData (GlEntObjId)

Update Staging.UKFinSfVolumeMonthly
Set PrimPyrAdjAmt = r.PrimPyrAdjAmt
	,PrimPyrBalAmt = r.PrimPyrBalAmt
	,PrimPyrPmtAmt = r.PrimPyrPmtAmt
	,BadDebtFlag = case when r.BDIind = 0 then 'N' else 'Y' end
From Staging.UKFinSfVolumeMonthly m
	left join #RcvData r on r.GlEntObjId = m.GlEntObjId
